...
class ESRCodec(Codec):
    ...
    def __init__(self, codec_name, search_space=None):
        ...
        """
        super(ESRCodec, self).__init__(codec_name, search_space)
            ...
            for conv_id in range(len(conv_num)):
                for grow_id in range(len(grow_rate)):
                    for out_id in range(grow_id, len(grow_rate)):
                        ...
                        func_prob.append(block_prob[block_id] * conv_prob[conv_id] * growth_prob[grow_id] * growth_prob[out_id])  # noqa: E501
    ...
    def get_para_block(self):
        ...
        for ind in range(len(self.func_type)):
            ...
            if key[0] == 'S':
                ...
                para = key[2]**2 * (9 * key[1] + 0.5 * (2 + key[1]) * (key[1] - 1)) + (key[1] + 1) * key[2] * key[3] / 4
            else:
    ...
    def get_flops_block(self):
        ...
        for ind in range(len(self.func_type)):
            ...
            for i in range(1, len(key)):
                key[i] = int(key[i])
            if key[0] == 'S':
                flops = (key[2]**2 * (9 * key[1] + 0.5 * (2 + key[1]) * (key[1] - 1)) + (key[1] + 1) * key[2] * key[3]) * 640 * 360  # noqa: E501
            elif key[0] == 'G':
                flops = (key[2]**2 * 9 * (3.5 + (4 + key[1]) * (key[1] - 3) / 8) + (key[1] + 1) * key[2] * key[3]) * 640 * 360  # noqa: E501
            elif key[0] == 'C':
                flops = ((key[2]**2 * (9 * key[1] * 3 + 0.5 * (2 + key[1]) * (key[1] - 1))) * 320 * 180 + ((key[1] + 1) * key[2] * key[3] / 4) * 640 * 360)  # noqa: E501
            else:
                logging.info('Wrong block type is used')
    ...
    def decode(self, indiv):
        ...
        indiv_cfg[model]['code'] = indiv.gene
