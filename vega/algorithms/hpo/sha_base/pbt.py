...
class PBT(object):
    ...
    def __init__(self, config_count, each_epochs, total_rungs, local_base_path, paras_list):
        """Init PBA."""
        self.total_rungs = total_rungs
        self.each_epochs = each_epochs
        self.config_count = config_count
        self.local_base_path = local_base_path
        self.hyperparameter_list = paras_list
        ...
        for i in range(self.config_count):
            config = self.hyperparameter_list[i]
    ...
    def explore(self, policy):
        ...
        for key, value in policy.items():
            if np.random.random() < 0.2:
                ...
                new_policy[key] = value * 1.1
            else:
                new_policy[key] = value * 0.9
        return new_policy
    ...
    def _init_next_rung(self):
        ...
        for i in range(4):
            ...
            better_worker_result_path = FileOps.join_path(self.local_base_path, 'cache',
                                                          str(better_id), 'checkpoint')
            FileOps.make_dir(better_worker_result_path)
            worse_worker_result_path = FileOps.join_path(self.local_base_path, 'cache',
